...
class Search_controller extends Controller
{
	...
	{
		if (Session::getValue('level') == 7 OR Session::getValue('level') == 10)
		{
				...
				if ($action == 'search')
				{
					...
					'<div class="searchp">
						<div class="data">
							...
							<br><strong>Précio secundario: </strong>' . ((!empty($price['pref_price'])) ? '$ ' . $price['pref_price'] . ' ' . $coin : '') . '
							<br><strong>Précio público: </strong>$ ' . $price['public_price'] . ' ' . $coin . '
							<br><strong>Descuento: </strong> ' . $discount . '
							<br><strong>Observaciones: </strong> ' . (!empty($product['observations']) ? $product['observations'] : '') . '
						</div>
						<div class="buttons">
							<form>
