'use strict';

menuActive('sales');

$(document).ready(function ()
{
    var myDocument = $(this);

    /* Lista de ventas
    /* ------------------------------------------------------------------------ */
    var salesTable = myDocument.find("#salesTable").DataTable({
        dom: 'Bfrtip',
        buttons: [

        ],
        "columnDefs": [
            {
                "orderable": true,
                "targets": "_all"
            },
            {
                "className": "text-left",
                "targets": "_all"
            }
        ],
        "order": [
            [2,'desc']
        ],
        "searching": true,
        "info":     true,
        "paging":   true,
        "language": {

        }
    });

    /* Lista de items de nueva venta
    /* ------------------------------------------------------------------------ */
    var addItemsSaleTable = myDocument.find("#addItemsSaleTable").DataTable({
        dom: 'Bfrtip',
        buttons: [

        ],
        "columns": [
            {
                "title": "Cant."
            },
            {
                "title": "Folio"
            },
            {
                "title": "Descripción"
            },
            {
                "title": "Precio"
            },
            {
                "title": "Descuento"
            },
            {
                "title": "Total"
            },
            {
                "title": ""
            }
        ],
        "columnDefs": [
            {
                "orderable": false,
                "targets": "_all"
            },
            {
                "className": "text-left",
                "targets": "_all"
            }
        ],
        "order": [

        ],
        "searching": false,
        "info":     false,
        "paging":   false,
        "language": {

        }
    });

    /* Lista de items venta en el ticket
    /* ------------------------------------------------------------------------ */
    var salesTicketTable = myDocument.find("#salesTicketTable").DataTable({
        dom: 'Bfrtip',
        buttons: [

        ],
        "columns": [
            {
                "title": "Cant.",
                "width": "10px"
            },
            {
                "title": "Desc."
            },
            {
                "title": "Pre."
            },
            {
                "title": "Desc."
            },
            {
                "title": "Tot."
            }
        ],
        "columnDefs": [
            {
                "orderable": false,
                "targets": "_all"
            },
            {
                "className": "text-left",
                "targets": "_all"
            }
        ],
        "order": [

        ],
        "searching": false,
        "info":     false,
        "paging":   false,
        "language": {

        }
    });

    /* Obtener todas las configuraciones iniciales
    /* ------------------------------------------------------------------------ */
    var mainCoin = '';
    var ivaRate = 0;
    var usdRate = 0;
    var saleTicketPrint = '';
    var saleTicketTotalsBreakdown = '';

    $.ajax({
        url: '/pointsale/getAllSalesSettings',
        type: 'POST',
        processData: false,
        cache: false,
        dataType: 'json',
        success: function(response)
        {
            mainCoin = response.data.main_coin;
            ivaRate = response.data.iva_rate;
            usdRate = response.data.usd_rate;
            saleTicketPrint = response.data.sale_ticket_print;
            saleTicketTotalsBreakdown = response.data.sale_ticket_totals_breakdown;
        }
    });

    /* Buscar producto o servicio para vender
    /* ------------------------------------------------------------------------ */
    var btnSearchToSell = $('[data-action="searchToSell"]');
    var frmSearchToSell = $('form[name="searchToSell"]');

    var total = 0;
    var mxnTotal = 0;
    var usdTotal = 0;
    var totalReceived = 0;
    var mxnTotalReceived = 0;
    var usdTotalReceived = 0;
    var change = 0;
    var payment = '';

    var sales = [];
    var salesDataJason = [];

    btnSearchToSell.on('click', function()
    {
        frmSearchToSell.submit();
    });

    frmSearchToSell.on('submit', function(e)
    {
        e.preventDefault();

        var self = $(this);
        var data = self.serialize();

        $.ajax({
            url: '/pointsale/searchToSell',
            type: 'POST',
            data: data,
            processData: false,
            cache: false,
            dataType: 'json',
            success: function(response)
            {
                checkValidateFormAjax(self, response, function()
                {
                    var additionalDiscount = response.additionalDiscount;

                    var quantity = response.data.quantity;
                    var folio = response.data.object.folio;
                    var description = response.data.object.name;

                    if (response.data.object.coin == '1')
                        var coin = 'MXN';
                    else if (response.data.object.coin == '2')
                        var coin = 'USD';

                    var price = response.data.totals.price;

                    if (response.data.totals.discount)
                        var discount = response.data.totals.discount;
                    else
                        var discount = '$ 0 ' + coin;

                    var discountQuantity = response.data.totals.discountQuantity;
                    var discountType = response.data.totals.discountType;

                    var totalPrice = response.data.totals.total;

                    if (additionalDiscount == true)
                    {
                        $('form[name="searchToSell"]').addClass('hidden');
                        $('form[name="applyAdditionalDiscount"]').removeClass('hidden');

                        $('input[name="dQuantity"]').val(quantity);
                        $('input[name="dDescription"]').val('[' + folio + '] ' + description);
                        $('input[name="dPrice"]').val('$ ' + price + ' ' + coin);
                        $('input[name="dTotal"]').val('$ ' + totalPrice + ' ' + coin);
                        $('input[name="additionalDiscountQuantity"]').val(discountQuantity);
                        $('select[name="additionalDiscountType"]').val(discountType);

                        salesDataJason = JSON.stringify(response.data);
                    }
                    else
                    {
                        var folioRepeat = false;

                        $('#addItemsSaleTable tr').each(function()
                        {
                            if ($(this).find('td').eq(1).text() == folio)
                                folioRepeat = true;
                        });

                        if (folioRepeat == false)
                        {
                            addItemsSaleTable.row.add([
                                quantity,
                                folio,
                                description,
                                '$ ' + price + ' ' + coin,
                                discount,
                                '$ <span class="' + coin + '">' + totalPrice + '</span> ' + coin,
                                '<a data-delete="' + folio + '"><i class="material-icons">delete</i></a>'
                            ]).draw();

                            sales.push(response.data);
                        }

                        mxnTotal = 0;
                        usdTotal = 0;

                        $('.MXN').each(function()
                        {
                            mxnTotal += parseFloat($(this).html()||0,10);
                        });

                        $('.USD').each(function()
                        {
                            usdTotal += parseFloat($(this).html()||0,10);
                        });

                        $('input[name="mxnTotal"]').val('$ ' + mxnTotal + ' MXN');
                        $('input[name="usdTotal"]').val('$ ' + usdTotal + ' USD');

                        if (mainCoin == 'MXN')
                        {
                            total = (usdTotal * usdRate) + mxnTotal;
                            $('input[name="total"]').val('$ ' + total + ' MXN');
                        }
                        else if (mainCoin == 'USD')
                        {
                            total = (mxnTotal / usdRate) + usdTotal;
                            $('input[name="total"]').val('$ ' + Math.ceil(total) + ' USD');
                        }

                        $('select[name="branchOffice"]').val('').trigger('chosen:updated');
                        $('input[name="quantity"]').val('1');
                        $('input[name="folio"]').val('');
                        $('input[name="folio"]').focus();
                    }
                });
            }
        });
    });

    var sappl = $('input[name="folio"]').data('sappl');
    var sfoli = $('input[name="folio"]').data('sfoli');
    var squan = $('input[name="folio"]').data('squan');
    var sbran = $('input[name="folio"]').data('sbran');
    var sdisc = $('input[name="folio"]').data('sdisc');

    if (sappl == true)
    {
        $('select[name="branchOffice"]').val(sbran).trigger('chosen:updated');
        $('input[name="quantity"]').val(squan);
        $('input[name="folio"]').val(sfoli);
        $('input[name="additionalDiscount"]').attr('checked', sdisc);
        btnSearchToSell.click();
    }

    /* Aplicar descuento extra
    /* ------------------------------------------------------------------------ */
    var btnApplyAdditionalDiscount = $('[data-action="applyAdditionalDiscount"]');
    var frmApplyAdditionalDiscount = $('form[name="applyAdditionalDiscount"]');

    btnApplyAdditionalDiscount.on('click', function()
    {
        frmApplyAdditionalDiscount.submit();
    });

    frmApplyAdditionalDiscount.on('submit', function(e)
    {
        e.preventDefault();

        var self = $(this);
        var data = self.serialize();

        $.ajax({
            url: '/pointsale/applyAdditionalDiscount',
            type: 'POST',
            data: data + '&data=' + salesDataJason,
            processData: false,
            cache: false,
            dataType: 'json',
            success: function(response)
            {
                checkValidateFormAjax(self, response, function()
                {
                    var quantity = response.data.quantity;
                    var folio = response.data.object.folio;
                    var description = response.data.object.name;

                    if (response.data.object.coin == '1')
                        var coin = 'MXN';
                    else if (response.data.object.coin == '2')
                        var coin = 'USD';

                    var price = response.data.totals.price;

                    if (response.data.totals.discount)
                        var discount = response.data.totals.discount;
                    else
                        var discount = '$ 0 ' + coin;

                    var totalPrice = response.data.totals.total;
                    var folioRepeat = false;

                    $('#addItemsSaleTable tr').each(function()
                    {
                        if ($(this).find('td').eq(1).text() == folio)
                            folioRepeat = true;
                    });

                    if (folioRepeat == false)
                    {
                        addItemsSaleTable.row.add([
                            quantity,
                            folio,
                            description,
                            '$ ' + price + ' ' + coin,
                            discount,
                            '$ <span class="' + coin + '">' + totalPrice + '</span> ' + coin,
                            '<a data-delete="' + folio + '"><i class="material-icons">delete</i></a>'
                        ]).draw();

                        sales.push(response.data);
                    }

                    mxnTotal = 0;
                    usdTotal = 0;

                    $('.MXN').each(function()
                    {
                        mxnTotal += parseFloat($(this).html()||0,10);
                    });

                    $('.USD').each(function()
                    {
                        usdTotal += parseFloat($(this).html()||0,10);
                    });

                    $('input[name="mxnTotal"]').val('$ ' + mxnTotal + ' MXN');
                    $('input[name="usdTotal"]').val('$ ' + usdTotal + ' USD');

                    if (mainCoin == 'MXN')
                    {
                        total = (usdTotal * usdRate) + mxnTotal;
                        $('input[name="total"]').val('$ ' + total + ' MXN');
                    }
                    else if (mainCoin == 'USD')
                    {
                        total = (mxnTotal / usdRate) + usdTotal;
                        $('input[name="total"]').val('$ ' + Math.ceil(total) + ' USD');
                    }

                    $('form[name="searchToSell"]').removeClass('hidden');
                    $('form[name="searchToSell"]')[0].reset();

                    $('form[name="applyAdditionalDiscount"]').addClass('hidden');
                    $('form[name="applyAdditionalDiscount"]')[0].reset();
                });
            }
        });
    });

    /* Eliminar servicio o producto de la lista de ventas
    /* ------------------------------------------------------------------------ */
    var btnCancelApplyAdditionalDiscount = $('[data-action="cancelApplyAdditionalDiscount"]');

    btnCancelApplyAdditionalDiscount.on('click', function(e)
    {
        $('form[name="searchToSell"]').removeClass('hidden');
        $('input[name="quantity"]').parent().parent().removeClass('hidden');
        $('input[name="folio"]').parent().parent().attr('class', 'input-group span10');
        document.getElementById('additionalDiscount').checked = false;

        $('form[name="applyAdditionalDiscount"]').addClass('hidden');
        $('form[name="applyAdditionalDiscount"]')[0].reset();

    });

    /* Eliminar servicio o producto de la lista de ventas
    /* ------------------------------------------------------------------------ */
    $(document).on('click', '[data-delete]', function ()
    {
        var self = $(this);
        var folio = self.data('delete');
        var row_table = self.parents('tr');

        if (confirm("¿Esta seguro que desea eliminar este elemento?") == true)
        {
            addItemsSaleTable.$('tr.selected').removeClass('selected');
            row_table.addClass('selected');
            addItemsSaleTable.row('.selected').remove().draw(false);

            sales = jQuery.grep(sales, function (data, key)
            {
                if (data['object']['folio'] != folio)
                    return data;
            });

            mxnTotal = 0;
            usdTotal = 0;

            $('.MXN').each(function()
            {
                mxnTotal += parseFloat($(this).html()||0,10);
            });

            $('.USD').each(function()
            {
                usdTotal += parseFloat($(this).html()||0,10);
            });

            $('input[name="mxnTotal"]').val('$ ' + mxnTotal + ' MXN');
            $('input[name="usdTotal"]').val('$ ' + usdTotal + ' USD');

            if (mainCoin == 'MXN')
            {
                total = (usdTotal * usdRate) + mxnTotal;
                $('input[name="total"]').val('$ ' + total + ' MXN');
            }
            else if (mainCoin == 'USD')
            {
                total = (mxnTotal / usdRate) + usdTotal;
                $('input[name="total"]').val('$ ' + Math.ceil(total) + ' USD');
            }

            $('input[name="quantity"]').val('1');
            $('input[name="folio"]').val('');
            $('input[name="folio"]').focus();
        }
    });

    /* Seleccionar tipo de pago
    /* ------------------------------------------------------------------------ */
    var sltPayment = $('select[name="payment"]');

    sltPayment.on('change', function()
    {
        if (sltPayment.val() == 'cash')
        {
            $('input[name="num_deferred_payments"]').val('2');
            $('input[name="num_deferred_payments"]').parent().parent().addClass('hidden');

            $('#deferred_payments').html('');
            $('#deferred_payments').removeClass('hidden');

            $('input[name="mxnTotalReceived"]').parent().parent().removeClass('hidden');
            $('input[name="usdTotalReceived"]').parent().removeClass('hidden');
            $('input[name="totalReceived"]').parent().parent().removeClass('hidden');
            $('input[name="change"]').parent().removeClass('hidden');
        }
        else if (sltPayment.val() == 'card')
        {
            $('input[name="num_deferred_payments"]').val('2');
            $('input[name="num_deferred_payments"]').parent().parent().addClass('hidden');

            $('#deferred_payments').html('');
            $('#deferred_payments').removeClass('hidden');

            $('input[name="mxnTotalReceived"]').parent().parent().addClass('hidden');
            $('input[name="usdTotalReceived"]').parent().addClass('hidden');
            $('input[name="totalReceived"]').parent().parent().addClass('hidden');
            $('input[name="change"]').parent().addClass('hidden');
        }
        else if (sltPayment.val() == 'deferred')
        {
            if ($('input[name="num_deferred_payments"]').val() < 2)
                var num_deferred_payments = 2;
            else
                var num_deferred_payments = $('input[name="num_deferred_payments"]').val();

            $('input[name="num_deferred_payments"]').val(num_deferred_payments);
            $('input[name="num_deferred_payments"]').parent().parent().removeClass('hidden');

            for (var i = 1; i <= num_deferred_payments; i++)
            {
                $('#deferred_payments').append('<fieldset class="input-group">'
                    + '<label data-important>'
                    + '<span>Pago ' + i + '</span>'
                    + '<input type="number" name="deferred_payment_' + i + '" value="' + (total / num_deferred_payments) + '" />'
                    + '</label>'
                    + '</fieldset>');
            }

            $('#deferred_payments').removeClass('hidden');

            $('input[name="mxnTotalReceived"]').parent().parent().removeClass('hidden');
            $('input[name="usdTotalReceived"]').parent().removeClass('hidden');
            $('input[name="totalReceived"]').parent().parent().removeClass('hidden');
            $('input[name="change"]').parent().removeClass('hidden');
        }

        $('input[name="mxnTotalReceived"]').val('');
        $('input[name="usdTotalReceived"]').val('');
        $('input[name="totalReceived"]').val('$ 0.00 ' + mainCoin);
        $('input[name="change"]').val('$ 0.00 ' + mainCoin);
    });

    $('input[name="num_deferred_payments"]').on('change', function()
    {
        if ($('input[name="num_deferred_payments"]').val() >= 2)
        {
            var num_deferred_payments = $('input[name="num_deferred_payments"]').val();

            $('#deferred_payments').html('');

            for (var i = 1; i <= num_deferred_payments; i++)
            {
                $('#deferred_payments').append('<fieldset class="input-group">'
                    + '<label data-important>'
                    + '<span>Pago ' + i + '</span>'
                    + '<input type="number" name="deferred_payment_' + i + '" value="' + (total / num_deferred_payments) + '" />'
                    + '</label>'
                    + '</fieldset>');
            }
        }
        else
        {
            alert('No pueden haber pagos diferidos menor a 2');
            $('input[name="num_deferred_payments"]').val('2')
        }
    });

    /* Hacer el pago de la venta
    /* ------------------------------------------------------------------------ */
    var btnMakePayment = $('[data-action="makePayment"]');
    var frmMakePayment = $('form[name="makePayment"]');

    btnMakePayment.on('click', function()
    {
        frmMakePayment.submit();
    });

    frmMakePayment.on('submit', function(e)
    {
        e.preventDefault();

        var self = $(this);
        var data = self.serialize();

        var salesJson = JSON.stringify(sales);

        $.ajax({
            url: '',
            type: 'POST',
            data: data + '&total=' + total + '&mxnTotal=' + mxnTotal + '&usdTotal=' + usdTotal + '&sales=' + salesJson,
            processData: false,
            cache: false,
            dataType: 'json',
            success: function(response)
            {
                checkValidateFormAjax(self, response, function()
                {
                    var folio = response.data.folio;
                    var date = response.data.date;
                    var user = response.data.user;
                    var client = response.data.client;
                    var branchOfficeName = response.data.branchOffice.name;
                    var branchOfficePhoneNumber = response.data.branchOffice.phoneNumber;
                    var branchOfficeAddress = response.data.branchOffice.address;
                    var branchOfficeFiscalName = response.data.branchOffice.fiscalName;
                    var branchOfficeFiscalCode = response.data.branchOffice.fiscalCode;
                    var branchOfficeFiscalRegime = response.data.branchOffice.fiscalRegime;
                    var branchOfficeFiscalAddress = response.data.branchOffice.fiscalAddress;

                    var subtotal = response.data.subtotal;
                    var iva = response.data.iva;

                    totalReceived = response.data.totalReceived;
                    mxnTotalReceived = response.data.mxnTotalReceived;
                    usdTotalReceived = response.data.usdTotalReceived;

                    if (response.data.payment == 'cash')
                        payment = 'Efectivo';
                    else if (response.data.payment == 'card')
                        payment = 'Tarjeta de crédito/débito';
                    else if (response.data.payment == 'deferred')
                        payment = 'Pagos diferidos';

                    change = response.data.change;

                    $('input[name="quantity"]').val();
                    $('input[name="quantity"]').attr('disabled', 'true');
                    $('input[name="quantity"]').parent().parent().addClass('hidden');

                    $('input[name="folio"]').val();
                    $('input[name="folio"]').attr('disabled', 'true');
                    $('input[name="folio"]').parent().parent().addClass('hidden');

                    $('[data-delete]').addClass('hidden');

                    $('select[name="payment"]').attr('disabled', 'true');

                    $('input[name="mxnTotalReceived"]').val('');
                    $('input[name="mxnTotalReceived"]').attr('disabled', 'true');
                    $('input[name="mxnTotalReceived"]').parent().parent().addClass('hidden');

                    $('input[name="usdTotalReceived"]').val('');
                    $('input[name="usdTotalReceived"]').attr('disabled', 'true');
                    $('input[name="usdTotalReceived"]').parent().parent().addClass('hidden');

                    if (response.data.payment == 'cash')
                    {
                        $('input[name="totalReceived"]').val('$ ' + totalReceived + ' ' + mainCoin);
                        $('input[name="change"]').val('$ ' + change + ' ' + mainCoin);
                    }
                    else if (response.data.payment == 'card')
                    {
                        $('input[name="totalReceived"]').parent().parent().addClass('hidden');
                        $('input[name="change"]').parent().addClass('hidden');
                    }

                    $('[data-action="newSale"]').removeClass('hidden');

                    if (saleTicketPrint == 'true')
                    {
                        $('[data-action="printTicket"]').removeClass('hidden');

                        $('#folio').html('Folio: ' + folio);
                        $('#date').html(date);
                        $('#user').html('Vendedor: ' + user);
                        $('#client').html('Cliente: ' + client);
                        $('#branchOfficeName').html(branchOfficeName);
                        $('#branchOfficePhoneNumber').html(branchOfficePhoneNumber);
                        $('#branchOfficeAddress').html(branchOfficeAddress);
                        $('#branchOfficeFiscalName').html(branchOfficeFiscalName);
                        $('#branchOfficeFiscalCode').html(branchOfficeFiscalCode);
                        $('#branchOfficeFiscalRegime').html(branchOfficeFiscalRegime);
                        $('#branchOfficeFiscalAddress').html(branchOfficeFiscalAddress);

                        for (var i = 0; i < sales.length; i++)
                        {
                            var quantity = sales[i]['quantity'];
                            var description = sales[i]['object']['name'];
                            var price = sales[i]['totals']['price'];

                            if (sales[i]['totals']['discount'])
                                var discount = sales[i]['totals']['discount'];
                            else
                                var discount = 0;

                            var totalPrice = sales[i]['totals']['total'];

                            if (sales[i]['object']['coin'] == '1')
                                var coin = 'MXN';
                            else if (sales[i]['object']['coin'] == '2')
                                var coin = 'USD';

                            salesTicketTable.row.add([
                                quantity,
                                description,
                                price,
                                discount,
                                '<span class="' + coin + '">' + totalPrice + '</span>'
                            ]).draw();
                        }

                        if (saleTicketTotalsBreakdown == 'true')
                        {
                            $('#subtotal').html('Subtotal: $ ' + subtotal + ' ' + mainCoin);
                            $('#iva').html('IVA: (' + ivaRate + '%) $ ' + iva + ' ' + mainCoin);
                        }

                        $('#total').html('Total a pagar: $ ' + total + ' ' + mainCoin);

                        if (response.data.payment == 'cash')
                        {
                            $('#totalReceived').html('Total recibido: $ ' + totalReceived + ' ' + mainCoin);
                            $('#mxnTotalReceived').html('($ ' + mxnTotalReceived + ' MXN)');
                            $('#usdTotalReceived').html('($ ' + usdTotalReceived + ' USD)');
                            $('#change').html('Cambio: $ ' + change + ' ' + mainCoin);
                        }
                        else if (response.data.payment == 'card')
                        {
                            $('#totalReceived').addClass('hidden');
                            $('#mxnTotalReceived').addClass('hidden');
                            $('#usdTotalReceived').addClass('hidden');
                            $('#change').addClass('hidden');
                        }
                        else if (response.data.payment == 'deferred')
                        {
                            $('#totalReceived').html('Total recibido: $ ' + totalReceived + ' ' + mainCoin);
                            $('#mxnTotalReceived').html('($ ' + mxnTotalReceived + ' MXN)');
                            $('#usdTotalReceived').html('($ ' + usdTotalReceived + ' USD)');
                            $('#change').addClass('hidden');

                            $('#deferred').html('<h4 style="font-family:arial;font-size:12px;text-transform:uppercase;text-align:right;padding:0px;margin:0px;color:#000;line-height:18px;">Pagos diferidos:</h4>');

                            var deferred_cile = 1;

                            for (var i = 0; i < response.data.deferred_payments_array.deferred_payments.length; i++)
                            {
                                $('#deferred_pays').append('<h4 style="font-family:arial;font-size:12px;text-transform:uppercase;text-align:right;padding:0px;margin:0px;color:#000;line-height:18px;">Pago ' + deferred_cile + ': $ ' + response.data.deferred_payments_array.deferred_payments[i].pay + ' ' + mainCoin + '</h4>');
                                deferred_cile = deferred_cile + 1;
                            }
                        }

                        $('#payment').html('Tipo de pago: ' + payment);
                    }
                });
            }
        });
    });

    /* Imprimir ticket
    /* ------------------------------------------------------------------------ */
    var btnPrintTicket = $('[data-action="printTicket"]');

    btnPrintTicket.on('click', function()
    {
        if (saleTicketPrint == 'true')
        {
            var ticket  = document.getElementById('ticket');
            var ventimp = window.open(' ','popimpr');
            ventimp.document.write(ticket.innerHTML);
            ventimp.document.close();
            ventimp.print();
            ventimp.close();
        }
    });

    /* Lista de productos y servicios
    /* ------------------------------------------------------------------------ */
    var tblProductsAndServices = myDocument.find("#tblProductsAndServices").DataTable({
        dom: 'Bfrtip',
        buttons: [

        ],
        "columnDefs": [
            {
                "orderable": true,
                "targets": "_all"
            },
            {
                "className": "text-left",
                "targets": "_all"
            }
        ],
        "order": [
            [1, 'asc']
        ],
        "searching": true,
        "info":     false,
        "paging":   true,
        "language": {

        }
    });

    /* Lista de productos y servicios
    /* ------------------------------------------------------------------------ */
    modal('productsAndServices', function(modal)
    {

    }, function(modal)
    {

    });

    /* Cargar folio para búsqueda
    /* ------------------------------------------------------------------------ */
    // $('#tblProductsAndServices').find('tbody').find('[data-action="loadFolioToSearch"]').on('click', function()
    // {
    //     alert('hola');
    //
    //     // var folio = $(this).data('folio');
    //     // $('input[name="folio"]').val(folio);
    //     // $('[data-modal="productsAndServices"]').removeClass('view');
    //     // $('body').removeClass('noscroll');
    // });

    /* Lista de items de venta
    /* ------------------------------------------------------------------------ */
    var itemsSaleTable = myDocument.find("#itemsSaleTable").DataTable({
        dom: 'Bfrtip',
        buttons: [

        ],
        "columnDefs": [
            {
                "orderable": false,
                "targets": "_all"
            },
            {
                "className": "text-left",
                "targets": "_all"
            }
        ],
        "order": [
            [1, 'desc']
        ],
        "searching": false,
        "info":     false,
        "paging":   false,
        "language": {

        }
    });

    /* Cancelar una venta
    /* ------------------------------------------------------------------------ */
    $('[data-action="abone"]').on('click', function()
    {
        $('form[name="abone"]').submit();
    });

    $('form[name="abone"]').on('submit', function(e)
    {
        e.preventDefault();

        var self = $(this);
        var data = self.serialize();

        $.ajax({
            url: '',
            type: 'POST',
            data: data + '&action=abone',
            processData: false,
            cache: false,
            dataType: 'json',
            success: function(response)
            {
                checkValidateFormAjax(self, response, function()
                {
                    location.reload();
                });
            }
        });
    });

    /* Cancelar una venta
    /* ------------------------------------------------------------------------ */
    var btnCancelSale = $('[data-action="cancelSale"]');

    btnCancelSale.on('click', function()
    {
        $.ajax({
            url: '',
            type: 'POST',
            data: 'action=cancel',
            processData: false,
            cache: false,
            dataType: 'json',
            success: function(response)
            {
                $('body').prepend('<div data-loader-ajax><div class="loader-01"></div></div>');
                location.reload();
            }
        });
    });
});
